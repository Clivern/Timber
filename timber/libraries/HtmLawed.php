 ...
 * HTMLawd 1.1.19
 *
 * This class don't load at run time but used only to parse comments
...
class HtmLawed {
    ...
    public static function hl($t, $C = 1, $S = array())
    {
        ...
        $C['anti_link_spam']     = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or \Timber\Libraries\HtmLawed::hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or \Timber\Libraries\HtmLawed::hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
        ...
        $GLOBALS['C'] = $C;
        $S            = is_array($S) ? $S : \Timber\Libraries\HtmLawed::hl_spec($S);
        if (isset($GLOBALS['S'])) {
            $reS = $GLOBALS['S'];
        }
        $GLOBALS['S'] = $S;
        ...
        if ($C['cdata'] or $C['comment']) {
            $t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', '\Timber\Libraries\HtmLawed::hl_cmtcd', $t);
        }
        $t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', '\Timber\Libraries\HtmLawed::hl_ent', str_replace('&', '&amp;', $t));
        ...
        // main
        $t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', '\Timber\Libraries\HtmLawed::hl_tag', $t);
        $t = $C['balance'] ? \Timber\Libraries\HtmLawed::hl_bal($t, $C['keep_bad'], $C['parent']) : $t;
        ...
        $t = $C['tidy'] ? \Timber\Libraries\HtmLawed::hl_tidy($t, $C['tidy'], $C['parent']) : $t;
        unset($C, $e);
        if (isset($reC)) {
            $GLOBALS['C'] = $reC;
        }
    ...
    public static function hl_spec($t)
    {
        ...
        for ($i = count(($t = explode(';', $t))); --$i >= 0;) {
            ...
            foreach (explode(',', $a) as $v) {
                ...
                if (isset($y[$x]['match']) && !\Timber\Libraries\HtmLawed::hl_regex($y[$x]['match'])) {
                    unset($y[$x]['match']);
                }
                if (isset($y[$x]['nomatch']) && !\Timber\Libraries\HtmLawed::hl_regex($y[$x]['nomatch'])) {
                    unset($y[$x]['nomatch']);
                }
            }
            if (!count($y) && !count($n)) {
    ...
    public static function hl_tag($t)
    {
        ...
        if ($C['make_tag_strict'] && isset($eD[$e])) {
            $trt = \Timber\Libraries\HtmLawed::hl_tag2($e, $a, $C['make_tag_strict']);
            if (!$e) {
                return (($C['keep_bad'] % 2) ? str_replace(array(
                    '<',
                    '>'
        ...
        foreach ($aA as $k => $v) {
            if (((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e]))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
                ...
                if ($k == 'style' && !$C['style_pass']) {
                    ...
                    $v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', '\Timber\Libraries\HtmLawed::hl_prot', $v);
                    ...
                    $v = \Timber\Libraries\HtmLawed::hl_prot($v, $k);
                    if ($k == 'href') { // X-spam
                ...
                if (isset($rl[$k]) && is_array($rl[$k]) && ($v = \Timber\Libraries\HtmLawed::hl_attrval($v, $rl[$k])) === 0) {
                    continue;
                }
                $a[$k] = str_replace('"', '&quot;', $v);
            }
    ...
    public static function kses($t, $h, $p = array('http', 'https', 'ftp', 'news', 'nntp', 'telnet', 'gopher', 'mailto'))
    {
        ...
        $C['hook']     = '\Timber\Libraries\HtmLawed::kses_hook';
        $C['schemes']  = '*:' . implode(',', $p);
        return \Timber\Libraries\HtmLawed::hl($t, $C, $h);
        // eof
    }
